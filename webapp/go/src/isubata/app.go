...
func getMessage(c echo.Context) error {
	...
	*/
	...
	// queryMessages と　jsonifyMessage をまとめる
	...
	for rows.Next() {
		...
		var message_created_at time.Time
		var content string

		err := rows.Scan(&message_id, &user_name, &display_name, &avatar_icon, &message_created_at, &content)
		if err != nil {
			return err
		}
		...
		r["user"] = u
		r["date"] = message_created_at.Format("2006/01/02 15:04:05")
		r["content"] = content
		// push front
...
func fetchUnread(c echo.Context) error {
	...
	*/
	...
	rows, err := db.Query("SELECT channel.id AS channel_id, CASE WHEN message_id IS NULL THEN "+
		"(SELECT COUNT(*) FROM message WHERE channel_id = channel.id) ELSE "+
		"(SELECT COUNT(*) FROM message WHERE channel_id = channel.id AND message_id < id) END AS unread "+
		"FROM channel LEFT JOIN haveread ON channel.id = haveread.channel_id AND user_id = ?", userID)
	if err != nil {
		return err
	}
	...
	for rows.Next() {
		...
		var unread int64
		err := rows.Scan(&channel_id, &unread)
		if err != nil {
			return err
		}
...
func getHistory(c echo.Context) error {
	...
	*/
	...
	messages := []MessageWithUser{}
	err = db.Select(&messages, "SELECT m.id, m.content, m.created_at, u.name, u.display_name, u.avatar_icon FROM message m INNER JOIN user u ON m.user_id = u.id WHERE m.channel_id = ? ORDER BY m.id DESC LIMIT ? OFFSET ?;", chID, N, (page-1)*N)
	if err != nil {
		return err
	}
